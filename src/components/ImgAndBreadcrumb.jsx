...
const ImgAndBreadcrumb = ({ imageSrc, imageAlt, breadcrumbItems, title }) => {
  ...
  return (
    <div className="relative flex items-end justify-center h-44 sm:h-[35rem]">
      ...
      <WordPullUp
        words={title}
        className="sm:text-6xl text-slate-200  absolute top-14 sm:top-[40%] leading-tight sm:leading-none bg-black/50 pb-2 sm:p-10 hover:bg-black/50 hover:text-white transition-all duration-300 ease-in-out hover:scale-105 px-2 font-extrabold tracking-wide text-center"
      />
            ...
            breadcrumbItems.map((item, index) => (
              <BreadcrumbItem key={index}>
                {item.href ? (
                  ...
                  <BreadcrumbPage className="text-base font-semibold text-slate-200">
                    {item.label}
                  </BreadcrumbPage>
                )}
              </BreadcrumbItem>
