import cricket1 from "../assets/sports/Cricket/img (1).webp";
import cricket2 from "../assets/sports/Cricket/img (2).webp";
import cricket3 from "../assets/sports/Cricket/img (3).webp";
import cricket4 from "../assets/sports/Cricket/img (4).webp";
import cricket5 from "../assets/sports/Cricket/img (5).webp";
import cricket6 from "../assets/sports/Cricket/img (6).webp";
import cricket7 from "../assets/sports/Cricket/img (7).webp";
import cricket8 from "../assets/sports/Cricket/img (8).webp";
import cricket9 from "../assets/sports/Cricket/img (9).webp";
import cricket10 from "../assets/sports/Cricket/img (10).webp";
import cricket11 from "../assets/sports/Cricket/img (11).webp";
import cricket12 from "../assets/sports/Cricket/img (12).webp";

import basketball1 from "../assets/sports/Basketball/img (1).webp";
import basketball2 from "../assets/sports/Basketball/img (2).webp";
import basketball3 from "../assets/sports/Basketball/img (3).webp";
import basketball4 from "../assets/sports/Basketball/img (4).webp";
import basketball5 from "../assets/sports/Basketball/img (5).webp";

import indoor1 from "../assets/sports/Indoor sports/img (1).webp";
import indoor2 from "../assets/sports/Indoor sports/img (2).webp";
import indoor3 from "../assets/sports/Indoor sports/img (3).webp";
import indoor4 from "../assets/sports/Indoor sports/img (4).webp";
import indoor5 from "../assets/sports/Indoor sports/img (5).webp";
import indoor6 from "../assets/sports/Indoor sports/img (6).webp";
import indoor7 from "../assets/sports/Indoor sports/img (7).webp";
import indoor8 from "../assets/sports/Indoor sports/img (8).webp";

import skating1 from "../assets/sports/Skating/img (1).webp";

import swimming1 from "../assets/sports/Swimming/img (1).webp";
import swimming2 from "../assets/sports/Swimming/img (2).webp";
import swimming3 from "../assets/sports/Swimming/img (3).webp";
import swimming4 from "../assets/sports/Swimming/img (4).webp";

import badminton1 from "../assets/sports/Badminton/img (1).webp";
import badminton2 from "../assets/sports/Badminton/img (2).webp";
import badminton3 from "../assets/sports/Badminton/img (3).webp";


import tennis1 from "../assets/sports/Tennis/img (1).webp";
import tennis2 from "../assets/sports/Tennis/img (2).webp";
import tennis3 from "../assets/sports/Tennis/img (3).webp";






export const sportsData = {
  cricket: {
    title: "Cricket at CBS",
    subtitle: "Developing champions through discipline and dedication",
    routeLink: "/boarding-life/sports-at-cbs/cricket",
    images: [
      cricket1,
      cricket2,
      cricket3,
      cricket4,
      cricket5,
      cricket6,
      cricket7,
      cricket8,
      cricket9,
      cricket10,
      cricket11,
      cricket12,
    ],
    description:
      "Our cricket program develops technical skills, strategic thinking and sportsmanship through professional coaching and regular matches on our well-maintained grounds.",
  },
  basketball: {
    title: "Basketball at CBS",
    subtitle: "Building teamwork and agility on the court",
    routeLink: "/boarding-life/sports-at-cbs/basketball",
    images: [
      "https://cbs.edustoke.com/wp-content/uploads/2024/11/572x400_21.jpg",
      // Add more basketball-related image URLs
      basketball1,
      basketball2,
      basketball3,
      basketball4,
      basketball5,
    ],
    description:
      "Students train in our indoor basketball court with experienced coaches, learning teamwork and agility while competing in inter-school tournaments.",
  },
  hockey: {
    title: "Hockey at CBS",
    subtitle: "Excellence in India's national sport",
    routeLink: "/boarding-life/sports-at-cbs/hockey",
    images: [
      "https://cbs.edustoke.com/wp-content/uploads/2024/11/572x400_23.jpg",
      // Add more hockey-related image URLs
    ],
    description:
      "Our hockey program emphasizes both individual skills and team tactics, with regular practice sessions and competitive matches on our full-size football field.",
  },
  volleyball: {
    title: "Volleyball at CBS",
    subtitle: "Fostering teamwork and coordination",
    routeLink: "/boarding-life/sports-at-cbs/volleyball",
    images: [
      "https://cbs.edustoke.com/wp-content/uploads/2024/11/572x400_10.jpg",
      // Add more volleyball-related image URLs
    ],
    description:
      "Our temperature-controlled volleyball court hosts year-round training programs for all skill levels, from beginners to competitive players.",
  },
  football: {
    title: "Football at CBS",
    subtitle: "Building endurance and team spirit",
    routeLink: "/boarding-life/sports-at-cbs/football",
    images: [
      "https://cbs.edustoke.com/wp-content/uploads/2024/11/sports-quiz-2019-14.jpg",
      // Add more football-related image URLs
    ],
    description:
      "Students participate in track and field events on our 400-meter track, developing speed, strength and endurance through structured training programs.",
  },
  "indoor-sports": {
    title: "Indoor Sports at CBS",
    subtitle: "Developing focus and precision",
    routeLink: "/boarding-life/sports-at-cbs/indoor-sports",
    images: [
      "https://cbs.edustoke.com/wp-content/uploads/2024/11/572x400_8.jpg",
      // Add more indoor sports-related image URLs
      indoor1,
      indoor2,
      indoor3,
      indoor4,
      indoor5,
      indoor6,
      indoor7,
      indoor8,
    ],
    description:
      "We offer facilities for table tennis, badminton, chess and carrom, providing students with diverse options for recreation and competitive play.",
  },
  skating: {
    title: "Skating at CBS",
    subtitle: "Developing balance and coordination",
    routeLink: "/boarding-life/sports-at-cbs/skating",
    images: [
      skating1,
    ],
    description:
      "Our skating program helps students develop balance, coordination and agility through professional instruction on our dedicated skating rink.",
  },
  swimming: {
    title: "Swimming at CBS",
    subtitle: "Developing water safety and swimming skills",
    routeLink: "/boarding-life/sports-at-cbs/swimming",
    images: [
      swimming1,
      swimming2,
      swimming3,
      swimming4,
    ],
    description:
      "Students learn proper swimming techniques and water safety in our temperature-controlled pool under the guidance of certified instructors.",
  },
  badminton: {
    title: "Badminton at CBS",
    subtitle: "Developing precision and agility",
    routeLink: "/boarding-life/sports-at-cbs/badminton",
    images: [
      badminton1,
      badminton2,
      badminton3,
    ],
    description:
      "Our indoor badminton courts provide year-round training facilities where students can develop their skills and compete in tournaments.",
  },
  tennis: {
    title: "Tennis at CBS",
    subtitle: "Developing precision and technique",
    routeLink: "/boarding-life/sports-at-cbs/tennis",
    images: [
      tennis1,
      tennis2,
      tennis3,
    ],
    description:
      "Students receive professional tennis coaching on our well-maintained courts, learning proper techniques and participating in competitive matches.",
  },
};
